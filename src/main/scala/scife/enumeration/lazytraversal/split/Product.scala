...
//  def touchableStrictPair[T, V](s1: TouchableEnum[T], s2: TouchableEnum[V]): LazyEnumFinite[(T, V)] =
//    (s1, s2) match {
//      case (Empty, _) | (_, Empty) =>
//        Empty
...
//      case (s1, s2) =>
//        new ProductFiniteStrictTuple(s1, s2) with Skippable[(T, V)] with Touchable[(T, V)]
//    }

  def touchable[T, V](s1: TouchableEnum[T], s2: TouchableEnum[V]): LazyEnumFinite[scalaz.LazyTuple2[T, V]] =
    (s1, s2) match {
      case (Empty, _) | (_, Empty) =>
        Empty
//      case (s1: Singleton[T], s2: Finite[V] with ResetIter[V]) =>
...
//        new ProductSingletonRight[V, T](s1, s2) with Touchable[scalaz.LazyTuple2[T, V]]
      case (s1, s2) =>
        new ProductFiniteLazyTuple(s1, s2)(null) with Touchable[scalaz.LazyTuple2[T, V]]
    }
