...
object Chain {
  ...
  def apply[I, O](s1: Enum[I], s2: Depend[I, O])(implicit ms: MemoizationScope): Finite[(I, O)] = {
    (s1, s2) match {
      case (Empty, _) => Empty
  ...
  def apply[I, O, R](s1: Enum[I], s2: Depend[I, O], combine: (I, O) => R)(implicit ms: MemoizationScope) = {
    (s1, s2) match {
      case (Empty, _) => Empty
      case (s: Singleton[I], df: DependFinite[_, _]) =>
  ...
  def apply[I, I2, O](s1: Enum[I], s2: Depend[I2, O], chain: I => I2)(implicit ms: MemoizationScope) = {
    (s1, s2) match {
      case (Empty, _) => Empty
  ...
  def fin[I, O](s1: Finite[I], s2: DependFinite[I, O])(implicit ms: MemoizationScope) = {
    (s1, s2) match {
      case (Empty, _) => Empty
      case (s: Singleton[I], df: DependFinite[_, _]) =>
        df.apply(s.el) map { (o: O) => (s.el, o) }
