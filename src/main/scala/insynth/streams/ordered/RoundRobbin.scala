...
private[streams] class InnerRoundRobbin[T] protected[streams] (iteratorsIn: => Seq[Iterator[IntegerWeightPair[T]]])
  extends HasLogger {
  
  protected var iterators = Array(iteratorsIn.map(_.buffered): _*)
    ...
    def loop(currentIndex: Int): Stream[(T, Int)] = {
      ...
      if (iterators.size == 1)
        ...
        if (nextIndex > -1)
          ...
          iterators(nextIndex).next #:: loop(nextIndex)
        else
