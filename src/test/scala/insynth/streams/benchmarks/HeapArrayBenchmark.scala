...
class HeapArrayBenchmark extends DependentMemoizedBenchmark[Int, Dependent[(Int, Set[Int]), Tree]]
  ...
  def constructEnumerator(ms: MemoizationScope) = {
    ...
    val setChooser = Producer.memoized(
      (self: EnumType, pair: (Int, Set[Int])) => {
        val (size, set) = pair

        if (size <= 0) e.Singleton(Leaf)
        else if (size == 1) e.WrapperArray(set map { v => Node(Leaf, v, Leaf) })
        else {
          val roots = Enum(set)

          val kept: Dependent[(Int, Set[Int]), Tree] = new InMapper(self, { (chosen: Int) =>
            (size - 1, set - chosen)
          })

          val leftOut: Dependent[(Int, Set[Int]), Tree] = new InMapper(self, { (leftOut: Int) =>
            (size, set - leftOut)
          })
        }
      })
