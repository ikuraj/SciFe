...
class BinarySearchTreeBenchmarkTest extends FunSuite with Matchers with GeneratorDrivenPropertyChecks
  ...
  test("correctness, bigger sizes", common.tags.SlowTest) {
    val ms = new scope.AccumulatingScope
    val enum = constructEnumeratorBenchmark(ms)
    ms.memoizations.size should be(1)
